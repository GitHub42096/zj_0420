...
public class StringUtils{
    ...
    private static Locale parseLocaleTokens(String localeString, String[] tokens) {
        if (hasLength(localeString) && tokens != null) {
            ...
            if (tokens.length > 2) {
                ...
                if (hasLength(variant) && variant.startsWith("_")) {
    ...
    public static String encodePassword(String password, String algorithm) {
        ...
        byte[] var6 = encodedPassword;
        int var7 = encodedPassword.length;

        for(int var8 = 0; var8 < var7; ++var8) {
            byte anEncodedPassword = var6[var8];
    ...
    public static <T> List<T> string2List(String str, String regex) {
        return (List)(isEmpty(str) ? new ArrayList() : (List)Arrays.stream(str.split(regex)).collect(Collectors.toList()));
    }
}
