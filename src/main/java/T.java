...
import static utils.ExcelUtil.getNum;
...
public class T {
    ...
    public static void main(String[] args) {
        ...
        String[] person = new String[]{"想", "测试", "研发", "产品", "销售"};
        String[] status = new String[]{"在线", "离线"};
        // 生成数据


        for (int j = 8000; j < 50000; j += 2800) {
            ArrayList<ArrayList<Object>> data = new ArrayList<ArrayList<Object>>();
            Set set = new HashSet();
            String path = "C:\\Users\\admin\\Desktop\\project\\csv\\project"+j+".csv";
            for (int i = 0; i < j; i++) {
                ArrayList<Object> oneLine = new ArrayList<Object>();
                SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
                oneLine.add(m); // 项目id
                oneLine.add("项目"+ m); // 项目名
                oneLine.add(status[new Random().nextInt(2)]); // 状态
                oneLine.add(getTel()); // 项目联系电话
                oneLine.add(simpleDateFormat.format(new Date())); // 项目上线日期
                m++;
                data.add(oneLine);
                StringBuffer stringBuffer = new StringBuffer("");
                for (Object o : oneLine) {
                    stringBuffer.append(o).append(",");
                }
                stringBuffer.deleteCharAt(stringBuffer.length() - 1);
                set.add(stringBuffer.toString());
            }
            //ExcelUtil.writeExcelXlsx(data, path);
            FileReAndWrUtil.writeFile(new File(path), set);
        }
    }
